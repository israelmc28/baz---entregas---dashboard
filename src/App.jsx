...
/* ─────────────────────────────────────────────────────
   SUPABASE — Reemplaza estos 2 valores con los tuyos
   Los encuentras en Supabase: Settings → API
───────────────────────────────────────────────────── */
const SUPABASE_URL = "https://aprhicvlaxqgwxbsaurj.supabase.co/rest/v1/";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFwcmhpY3ZsYXhxZ3d4YnNhdXJqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA0NDYxNDIsImV4cCI6MjA5NjAyMjE0Mn0.x5BcPfb0s4nnkE0cib4C0jPE75JHjf3qyhbFkLGuAbQ";

async function dbFetch(method, table, body, filter="") {
...
const dbGet   = (t, f="")   => dbFetch("GET",    t, null, f);
const dbPost  = (t, b)      => dbFetch("POST",   t, b,    "");
const dbPatch = (t, b, f)   => dbFetch("PATCH",  t, b,    f);
...
const C = {
  ...
  negro:"#0D0D0D", g1:"#1A1A1A", g2:"#222222", g3:"#2E2E2E", g4:"#3A3A3A",
  txt:"#F2F2F2", sub:"#888888", verde:"#3CB371", rojo:"#E05252",
...
───────────────────────────────────────────────────── */
const USUARIOS = [
  { email:"israel@bazentregas.com",   pass:"baz2026", nombre:"Israel",   rol:"admin"    },
  { email:"isaias@bazentregas.com",   pass:"op1234",  nombre:"Isaias",   rol:"operador" },
  { email:"dafne@bazentregas.com",    pass:"op1234",  nombre:"Dafne",    rol:"operador" },
  { email:"fernanda@bazentregas.com", pass:"op1234",  nombre:"Fernanda", rol:"operador" },
  { email:"arturo@bazentregas.com",   pass:"op1234",  nombre:"Arturo",   rol:"operador" },
];
...
function Login({ onLogin }) {
  ...
  function go() {
    ...
    u ? onLogin(u) : setErr("Correo o contrasena incorrectos");
  }

  return (
    <div style={{minHeight:"100vh",background:C.negro,display:"flex",flexDirection:"column",
      alignItems:"center",justifyContent:"center",padding:24,fontFamily:"Arial,sans-serif"}}>
      <Logo width={150}/>
      <div style={{marginTop:28,width:"100%",maxWidth:380,background:C.g2,borderRadius:16,padding:24}}>
        <p style={{textAlign:"center",fontSize:13,color:C.sub,marginBottom:20}}>
        ...
        <select value={email} onChange={e=>setEmail(e.target.value)}
          ...
          {USUARIOS.map(u=>(
            <option key={u.email} value={u.email}>{u.nombre} ({u.rol})</option>
          ))}
        </select>
        <input value={pass} onChange={e=>setPass(e.target.value)} type="password"
          placeholder="Contrasena" onKeyDown={e=>e.key==="Enter"&&go()}
          style={{width:"100%",background:C.g3,border:"1px solid "+C.g4,borderRadius:10,
        ...
        <div style={{marginTop:14,background:C.g3,borderRadius:10,padding:"10px 14px"}}>
          ...
          <p style={{fontSize:11,color:C.txt}}>Israel (admin): israel@bazentregas.com / baz2026</p>
          <p style={{fontSize:11,color:C.txt,marginTop:3}}>Equipo: [nombre]@bazentregas.com / op1234</p>
        </div>
      </div>
    </div>
  );
...
function Dashboard({ plan, cargas, fechaPlan, isDesktop }) {
  ...
  const PanelKpis = (
    <div style={{background:"linear-gradient(160deg,#111,"+C.g1+")",
      border:"1.5px solid "+C.naranja+"44",borderRadius:16,
      padding:isDesktop?20:16, marginBottom:isDesktop?0:14}}>
      {/* Header */}
      ...
      <div style={{background:C.g2,borderRadius:12,padding:"12px 14px",marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:isDesktop?13:11,color:C.sub,letterSpacing:1,textTransform:"uppercase"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        {KPIs.map(k=>(
          <div key={k.label} style={{background:C.g2,borderRadius:12,
            padding:isDesktop?"16px 18px":"12px 14px",borderLeft:"3px solid "+k.color}}>
  ...
  const PanelLineas = (
    <div>
      <div style={{background:C.g2,borderRadius:12,padding:isDesktop?20:16,marginBottom:12}}>
        <p style={{fontSize:9,letterSpacing:3,color:C.sub,textTransform:"uppercase",marginBottom:12}}>
          Por Linea Transportista
        </p>
          ...
          return (
            <div key={cfg.nombre} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid "+C.g3}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {[
                  {val:tCol,  label:"Colocadas",  color:C.azul},
                  {val:ts,    label:"Surtidas",    color:C.amarillo},
                  {val:tp-tc, label:"Pendientes",  color:C.rojo},
                ].map(k=>(
                  <div key={k.label} style={{flex:1,background:C.g3,borderRadius:7,padding:"5px 8px",textAlign:"center"}}>
                    <div style={{fontSize:isDesktop?16:14,fontWeight:900,color:k.color}}>{k.val}</div>
                    <div style={{fontSize:8,color:C.sub}}>{k.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{background:isDesktop?"#4A9EFF11":"#4A9EFF11",
        border:"1px solid #4A9EFF33",borderRadius:10,
        padding:isDesktop?"12px 16px":"10px 14px",
        display:"flex",gap:isDesktop?10:8,alignItems:"center"}}>
        <span style={{fontSize:isDesktop?20:18}}>{isDesktop?"💻":"📸"}</span>
        <p style={{fontSize:isDesktop?12:11,color:C.sub,lineHeight:1.5}}>
          {isDesktop
            ? "Usa Ctrl+P para guardar como PDF o Windows+Shift+S para captura de pantalla."
            : "Captura el panel de arriba para compartirlo. iOS: boton lateral + subir volumen. Android: encendido + bajar volumen."}
        </p>
      </div>
    </div>
  );
...
/* ─────────────────────────────────────────────────────
   PANEL DE CARGAS
───────────────────────────────────────────────────── */
function PanelCargas({ cargas, setCargas, plan, setPlan, hist, setHist, user, guardarCarga, guardarFilaPlan, agregarHist, isDesktop }) {
  ...
  function agregarCarga() {
    ...
    guardarCarga(nueva, true);
    agregarHist({accion:"nueva_carga",transportista:trans,capacidad:parseInt(cap),
      cantidad:1,usuario:user.nombre,ts:Date.now(),folio:nueva.folio,destino});
    setFolio(""); setCompl(""); setVista("lista");
  }

  function actualizarCarga(id, cambios) {
    const prev = cargas.find(c=>c.id===id);
    const actualizada = {...prev,...cambios};
    setCargas(prev=>prev.map(c=>c.id===id?actualizada:c));
    if(cambios.estado && cambios.estado!==prev.estado) {
      const np = plan.map(r=>{
        ...
        if(Object.keys(d).length){ const nr={...r,...d}; guardarFilaPlan(nr); return nr; }
        return r;
      });
      setPlan(np);
      agregarHist({accion:"estado_"+(cambios.estado||"actualizada"),
        transportista:prev.transportista,capacidad:prev.capacidad,
        cantidad:1,usuario:user.nombre,ts:Date.now(),folio:prev.folio});
    }
    guardarCarga(actualizada, false);
  }
  ...
  const filtradas = cargas.filter(c=>{
    ...
    const matchB = c.folio.includes(q)||c.complemento.includes(q)||(c.economico||"").includes(q);
  ...
  if(vista==="detalle"&&cargaAct) {
    const c = cargaAct;
    return (
      <div style={{padding:"12px 12px 40px",maxWidth:isDesktop?700:"none",margin:isDesktop?"0 auto":"0"}}>
        ...
        <div style={{background:C.g2,borderRadius:14,padding:isDesktop?24:16}}>
          {/* Encabezado */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
            <div>
              <div style={{fontSize:isDesktop?22:19,fontWeight:900,color:coloresTrans[c.transportista]||C.naranja}}>{c.folio}</div>
              ...
              <div style={{display:"inline-block",marginTop:5,
                background:(destiColor[c.destino]||C.sub)+"22",
                border:"1px solid "+(destiColor[c.destino]||C.sub)+"55",
                borderRadius:7,padding:"3px 10px",fontSize:11,fontWeight:700,color:destiColor[c.destino]||C.sub}}>
                {c.destino}
              </div>
            </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <input value={c.economico||""} placeholder="Ej. ECO-1234"
              ...
              style={{width:"100%",background:C.g3,
                border:"1px solid "+(c.economico?C.azul:C.g4),
                borderRadius:9,padding:"11px 12px",color:C.txt,fontSize:15,fontWeight:700,
                outline:"none",opacity:user.rol!=="admin"?0.6:1}}/>
            {!c.economico&&(
              <p style={{fontSize:10,color:C.rojo,marginTop:4}}>
                Sin economico: esta unidad no cuenta como colocada
              </p>
            )}
          </div>
          {/* Destino — admin */}
              ...
              <div style={{display:"flex",gap:8}}>
                {["pendiente","cargada","surtida"].map(est=>(
                  <button key={est} onClick={()=>actualizarCarga(c.id,{estado:est})}
                    style={{flex:1,padding:"9px 4px",borderRadius:9,cursor:"pointer",
              ...
              {user.rol==="admin"&&c.fotos.length<3&&(
                <>
                  <button onClick={()=>fileRef.current.click()}
                    style={{padding:"6px 12px",borderRadius:8,cursor:"pointer",
                      border:"1px solid "+C.naranja+"55",background:C.naranja+"11",
                      color:C.naranja,fontSize:11,fontWeight:700}}>
                    Agregar foto
                  </button>
  ...
  if(vista==="nueva") {
    ...
    return (
      <div style={{padding:"12px 12px 40px",maxWidth:isDesktop?560:"none",margin:isDesktop?"0 auto":"0"}}>
        ...
        <div style={{background:C.g2,borderRadius:14,padding:isDesktop?24:16}}>
          ...
          {[
            {label:"Folio de carga",val:folio,   set:setFolio, ph:"Ej. CS03286750", big:true},
            {label:"Complemento",   val:complemento,set:setCompl,ph:"Ej. CS00315678",big:false},
          ].map(f=>(
            <div key={f.label} style={{marginBottom:12}}>
              <label style={{fontSize:10,color:C.sub,letterSpacing:2,textTransform:"uppercase"}}>{f.label}</label>
              <input value={f.val} onChange={e=>f.set(e.target.value)} placeholder={f.ph}
                style={{width:"100%",background:C.g3,border:"1px solid "+C.g4,borderRadius:9,
                  padding:"11px 12px",color:C.txt,fontSize:f.big?15:14,
                  fontWeight:f.big?700:400,outline:"none",marginTop:4}}/>
            </div>
          ))}
              ...
              {DESTINOS.map(d=>(
                <button key={d} onClick={()=>setDestino(d)}
                  style={{padding:"7px 12px",borderRadius:9,cursor:"pointer",
                    border:"2px solid "+(destino===d?(dc[d]||C.naranja):C.g4),
                    background:destino===d?(dc[d]||C.naranja)+"22":C.g3,
                    color:destino===d?(dc[d]||C.naranja):C.sub,
          ...
          <div style={{marginBottom:12}}>
            ...
            <select value={trans} onChange={e=>{setTrans(e.target.value);setCap(capsPorTrans[e.target.value][0]);}}
              style={{width:"100%",background:C.g3,border:"1px solid "+C.g4,borderRadius:9,
                padding:"11px 12px",color:C.txt,fontSize:14,outline:"none",marginTop:4}}>
              {TRANSPORTISTAS_CFG.map(t=><option key={t.nombre} value={t.nombre}>{t.icon} {t.nombre}</option>)}
            </select>
          </div>
  ...
  /* ── LISTA ── */
  return (
    <div style={{padding:"12px 12px 40px",maxWidth:isDesktop?900:"none",margin:isDesktop?"0 auto":"0"}}>
      {/* Búsqueda + botón */}
        ...
        {["TODOS",...DESTINOS].map(d=>{
          ...
          return (
            <button key={d} onClick={()=>setFiltroDestino(d)}
              style={{padding:"5px 11px",borderRadius:20,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,
      ...
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {["TODOS","pendiente","cargada","surtida"].map(e=>(
          <button key={e} onClick={()=>setFiltroEstado(e)}
            style={{padding:"5px 11px",borderRadius:20,cursor:"pointer",whiteSpace:"nowrap",
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,marginBottom:12}}>
        {[
          ...
          <div key={k.l} style={{background:C.g2,borderRadius:9,padding:"7px 4px",
            textAlign:"center",borderTop:"2px solid "+k.col}}>
            ...
            <div key={c.id} onClick={()=>{setCargaId(c.id);setVista("detalle");}}
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,marginLeft:8}}>
                  ...
                  <span style={{fontSize:12,color:C.g4}}>›</span>
                </div>
              </div>
            </div>
          );
...
/* ─────────────────────────────────────────────────────
   HISTORIAL
───────────────────────────────────────────────────── */
function Historial({ hist, isDesktop }) {
  ...
  return (
    <div style={{padding:"12px 12px 40px",maxWidth:isDesktop?800:"none",margin:isDesktop?"0 auto":"0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isDesktop?"1fr 1fr":"1fr",gap:7}}>
        {hist.map((r,i)=>(
          <div key={i} style={{background:C.g2,borderRadius:11,padding:"10px 14px",
            ...
            <div style={{fontSize:11,color:C.sub,marginTop:3}}>
              ...
              {r.destino&&<span style={{marginRight:6}}>{r.destino}</span>}
...
/* ─────────────────────────────────────────────────────
   PANEL CONFIGURACION (solo admin)
───────────────────────────────────────────────────── */
function PanelConfig({ plan, fechaPlan, onGuardarFecha, onGuardarPlan, isDesktop }) {
  ...
  return (
    <div style={{padding:"12px 12px 40px",maxWidth:isDesktop?700:"none",margin:isDesktop?"0 auto":"0"}}>
      ...
      {/* Fecha */}
      <div style={{background:C.g2,borderRadius:12,padding:isDesktop?20:16,marginBottom:14}}>
      ...
      <div style={{background:C.g2,borderRadius:12,padding:isDesktop?20:16,marginBottom:16}}>
          ...
          return (
            <div key={nom} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid "+C.g3}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                ...
                <span style={{fontSize:16,fontWeight:900,color}}>{nom}</span>
              </div>
              {rows.map(r=>(
                <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                  <div style={{flex:1,background:C.g3,borderRadius:8,padding:"8px 12px"}}>
                    ...
                    <span style={{fontSize:10,color:C.sub,marginLeft:8}}>
                      {r.cargadas} carg. / {r.surtidas} surt.
                    </span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <button onClick={()=>cambiarPlan(r.id,r.plan-1)}
                      style={{width:32,height:32,borderRadius:8,border:"none",cursor:"pointer",
                        background:r.plan<=0?C.g4:C.rojo+"33",
                        color:r.plan<=0?C.g4:C.rojo,fontSize:18,fontWeight:700}}>-</button>
                    <input type="number" value={r.plan} onChange={e=>cambiarPlan(r.id,e.target.value)}
                      style={{width:52,textAlign:"center",background:C.g3,
                        ...
                        padding:"6px 4px",color:C.txt,fontSize:16,fontWeight:900,outline:"none"}}/>
                    <button onClick={()=>cambiarPlan(r.id,r.plan+1)}
                      style={{width:32,height:32,borderRadius:8,border:"none",cursor:"pointer",
                        background:C.verde+"33",color:C.verde,fontSize:18,fontWeight:700}}>+</button>
                  </div>
                  <span style={{fontSize:10,color:C.sub,minWidth:24}}>uds</span>
                </div>
              ))}
            </div>
          );
      ...
      <button onClick={guardarTodo}
        style={{width:"100%",padding:14,borderRadius:12,border:"none",cursor:"pointer",
          background:guardado
            ?"linear-gradient(90deg,#2d7a4f,"+C.verde+")"
            :"linear-gradient(90deg,"+C.naranjaD+","+C.naranja+")",
      ...
      <p style={{fontSize:10,color:C.sub,textAlign:"center",marginTop:10,lineHeight:1.5}}>
        Los cambios se aplican para todos los usuarios en tiempo real.
      </p>
    </div>
  );
}
...
export default function App() {
  ...
  const cargarDatos = useCallback(async () => {
    try {
      const [rPlan,rCargas,rHist,rFecha] = await Promise.all([
        dbGet("baz_plan",   "?order=capacidad.asc"),
        dbGet("baz_cargas", "?order=ts.desc"),
        dbGet("baz_hist",   "?order=ts.desc&limit=100"),
        dbGet("baz_config", "?clave=eq.fecha"),
      ]);
      if(Array.isArray(rPlan)   && rPlan.length)  setPlan(rPlan);
      if(Array.isArray(rCargas))                   setCargas(rCargas);
      if(Array.isArray(rHist))                     setHist(rHist);
      if(Array.isArray(rFecha)  && rFecha.length)  setFechaPlan(rFecha[0].valor);
      setDbOk(true);
  ...
  async function guardarCarga(carga, esNueva) {
    setSaving(true);
    try {
      if(esNueva) await dbPost("baz_cargas", carga);
      else        await dbPatch("baz_cargas", carga, "?id=eq."+carga.id);
    } catch(_){}
    setSaving(false); setLastSync(new Date());
  }

  async function guardarFilaPlan(fila) {
    setSaving(true);
    try { await dbPatch("baz_plan", fila, "?id=eq."+fila.id); } catch(_){}
    setSaving(false); setLastSync(new Date());
  }

  async function guardarPlanCompleto(np) {
    setSaving(true); setPlan(np);
    try { await Promise.all(np.map(r=>dbPatch("baz_plan",r,"?id=eq."+r.id))); } catch(_){}
    setSaving(false); setLastSync(new Date());
  }

  async function guardarFecha(f) {
    setFechaPlan(f);
    try { await dbPatch("baz_config",{valor:f},"?clave=eq.fecha"); } catch(_){}
  }

  async function agregarHist(entrada) {
    try {
      await dbPost("baz_hist", entrada);
      setHist(prev=>[entrada,...prev].slice(0,100));
    } catch(_){}
  }
  ...
  return (
    <div style={{background:C.negro,minHeight:"100vh",fontFamily:"Arial,sans-serif",color:C.txt,
      ...
      {/* HEADER */}
      <div style={{background:"linear-gradient(160deg,#111,"+C.g1+")",
        borderBottom:"2.5px solid "+C.naranja,
        padding:isDesktop?"14px 32px 12px":"14px 16px 12px",
        position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
          maxWidth:isDesktop?1400:"none",margin:"0 auto",marginBottom:10}}>
          <Logo width={isDesktop?150:115}/>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:10,color:user.rol==="admin"?C.naranja:C.sub,fontWeight:700}}>
              {user.rol==="admin"?"👑":"👤"} {user.nombre}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6,justifyContent:"flex-end",marginTop:3}}>
              ...
              {lastSync&&!saving&&<span style={{fontSize:9,color:C.verde}}>✓ {hora(lastSync)}</span>}
        ...
        <div style={{background:C.g2,borderRadius:10,padding:"9px 13px",
          maxWidth:isDesktop?1400:"none",margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            <span style={{fontSize:10,color:C.sub,letterSpacing:1,textTransform:"uppercase"}}>
              Avance · {fechaPlan}
            </span>
      ...
      {/* BANNER ERROR DB */}
      {dbOk===false&&(
        <div style={{background:C.rojo+"22",border:"1px solid "+C.rojo,borderRadius:10,
          padding:"10px 16px",margin:"12px 12px 0",fontSize:12,color:C.rojo,display:"flex",gap:8}}>
          ⚠️ Sin conexion a la base de datos. Verifica SUPABASE_URL y SUPABASE_KEY en App.jsx
        </div>
      )}

      {/* TABS */}
      <div style={{display:"flex",gap:6,
        padding:isDesktop?"10px 32px 8px":"10px 12px 8px",
        position:"sticky",top:isDesktop?114:142,
        background:C.negro,zIndex:50,borderBottom:"1px solid "+C.g2}}>
        <div style={{display:"flex",gap:6,maxWidth:isDesktop?600:"none",
          width:"100%",margin:isDesktop?"0 auto":"0"}}>
          {TABS.map(t=>(
      ...
      {/* CONTENIDO */}
      {tab==="dashboard"&&(
        <Dashboard plan={plan} cargas={cargas} fechaPlan={fechaPlan} isDesktop={isDesktop}/>
      )}
      {tab==="cargas"&&(
        <PanelCargas cargas={cargas} setCargas={setCargas}
          ...
          guardarFilaPlan={guardarFilaPlan}
          agregarHist={agregarHist}
          isDesktop={isDesktop}/>
      )}
      {tab==="historial"&&(
        <Historial hist={hist} isDesktop={isDesktop}/>
      )}
      {tab==="config"&&user.rol==="admin"&&(
        <PanelConfig plan={plan} fechaPlan={fechaPlan}
          onGuardarFecha={guardarFecha}
          onGuardarPlan={guardarPlanCompleto}
          isDesktop={isDesktop}/>
      )}
